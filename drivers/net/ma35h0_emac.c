...
static int emac_ma35h0_ofdata_to_platdata(struct udevice *dev)
{
	struct emac_ma35h0_platdata *pdata = dev_get_platdata(dev);
...
static int emac_ma35h0_probe(struct udevice *dev)
{
	struct emac_ma35h0_platdata *pdata = dev_get_platdata(dev);
	...
	ret = dev_read_phandle_with_args(dev, "nuvoton,ma35h0-sys", NULL,
					 0, 0, &args);
	if (ret) {
...
static int ma35h0_read_rom_hwaddr(struct udevice *dev)
{
	struct emac_ma35h0_platdata *pdata = dev_get_platdata(dev);
	...
	regmap_read(pdata->regmap,
		    ...
		    &reg);
	eth_pdata->enetaddr[0] = reg & 0xFF;
	eth_pdata->enetaddr[1] = (reg >> 8) & 0xFF;
	eth_pdata->enetaddr[2] = (reg >> 16) & 0xFF;
	eth_pdata->enetaddr[3] = (reg >> 24) & 0xFF;
	regmap_read(pdata->regmap,
		    pdata->id == 0 ? REG_SYS_MACAD0HSR : REG_SYS_MACAD1HSR,
		    &reg);
	eth_pdata->enetaddr[4] = reg & 0xFF;
	eth_pdata->enetaddr[5] = (reg >> 8) & 0xFF;

	return !is_valid_ethaddr(eth_pdata->enetaddr);
}

const struct eth_ops emac_ma35h0_eth_ops = {
	...
	.read_rom_hwaddr	= ma35h0_read_rom_hwaddr,
};

static const struct udevice_id ma35h0_emac_ids[] = {
	{ .compatible = "nuvoton,ma35h0-emac" },
	{ }
};

U_BOOT_DRIVER(eth_emac_ma35h0) = {
	.name	= "emac_ma35h0",
	.id	= UCLASS_ETH,
	.of_match = ma35h0_emac_ids,
	.ofdata_to_platdata = emac_ma35h0_ofdata_to_platdata,
	.probe	= emac_ma35h0_probe,
	.ops	= &emac_ma35h0_eth_ops,
	.priv_auto_alloc_size = sizeof(struct dw_eth_dev),
	.platdata_auto_alloc_size = sizeof(struct emac_ma35h0_platdata),
