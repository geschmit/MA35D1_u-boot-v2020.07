...
static ulong ma35d0_fb_cal_pixel_clk_rate(ulong PllSrcClk, ulong rate,
        u32 *u32Reg)
{
	u32 u32M, u32N, u32P;
...
/* Initialize MA35D0 disp registers */
static void ma35d0_disp_register_init(struct ma35d0_lcd_info *disp_info,
                                      void __iomem *regs)
{
	u32 data;
...
static int ma35d0_fb_video_probe(struct udevice *dev)
{
	struct video_uc_platdata *plat = dev_get_uclass_platdata(dev);
	struct video_priv *uc_priv = dev_get_uclass_priv(dev);
	struct ma35d0_fb_priv *priv = dev_get_priv(dev);
	struct regmap *regmap, *regmap_sys;
	u32 u32Reg[2] = {0};
	struct ofnode_phandle_args args;
	ofnode node;
	struct display_timing timings;
	struct ma35d0_lcd_info disp_info;
	...
	node = ofnode_by_compatible(ofnode_null(), "nuvoton,ma35d0-fb");
	if (!ofnode_valid(node)) {
		dev_err(dev, "missing 'nuvoton,ma35d0-fb' node\n");
		return -ENXIO;
	}
	...
	if(CONFIG_MA35D0_DCU_ADDR != 0)
		plat->base = CONFIG_MA35D0_DCU_ADDR;
	...
	priv->pixclock.rate = ma35d0_fb_cal_pixel_clk_rate(24000000,
	...
	if (ret) {
		dev_err(dev, "missing ma35d0_disp hardware reset\n");
		return ret;
	}
	...
	ma35d0_disp_register_init(&disp_info, priv->regs);
...
static int ma35d0_fb_video_bind(struct udevice *dev)
{
...
static const struct udevice_id ma35d0_fb_ids[] = {
	{.compatible = "nuvoton,ma35d0-fb"},
	{ }
};

U_BOOT_DRIVER(ma35d0_fb) = {
	.name	= "ma35d0_fb",
	.id	= UCLASS_VIDEO,
	.of_match = ma35d0_fb_ids,
	.probe	= ma35d0_fb_video_probe,
	.bind	= ma35d0_fb_video_bind,
	.priv_auto_alloc_size	= sizeof(struct ma35d0_fb_priv),
