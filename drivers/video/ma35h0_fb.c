...
static int ma35h0_fb_video_probe(struct udevice *dev)
{
	...
	node = ofnode_by_compatible(ofnode_null(), "nuvoton,ma35h0-fb");
	if (!ofnode_valid(node)) {
		dev_err(dev, "missing 'nuvoton,ma35h0-fb' node\n");
		return -ENXIO;
	}
	...
	if(CONFIG_MA35H0_DCU_ADDR != 0)
		plat->base = CONFIG_MA35H0_DCU_ADDR;
	...
	if (ret) {
		dev_err(dev, "missing ma35h0_disp hardware reset\n");
		return ret;
	}
...
static int ma35h0_fb_video_bind(struct udevice *dev)
{
...
static const struct udevice_id ma35h0_fb_ids[] = {
	{.compatible = "nuvoton,ma35h0-fb"},
	{ }
};

U_BOOT_DRIVER(ma35h0_fb) = {
	.name	= "ma35h0_fb",
	.id	= UCLASS_VIDEO,
	.of_match = ma35h0_fb_ids,
	.probe	= ma35h0_fb_video_probe,
	.bind	= ma35h0_fb_video_bind,
	.priv_auto_alloc_size	= sizeof(struct ma35h0_fb_priv),
