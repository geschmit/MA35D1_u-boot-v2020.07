...
static struct nand_ecclayout ma35d0_nand_oob;
...
static void ma35d0_layout_oob_table(struct nand_ecclayout *pNandOOBTbl,
...
static void ma35d0_hwcontrol(struct mtd_info *mtd, int cmd, unsigned int ctrl)
{
	struct nand_chip *nand = mtd_to_nand(mtd);
	struct ma35d0_nand_info *nand_info = nand_get_controller_data(nand);
...
/* select chip */
static void ma35d0_nand_select_chip(struct mtd_info *mtd, int chip)
{
	struct nand_chip *nand = mtd_to_nand(mtd);
	struct ma35d0_nand_info *nand_info = nand_get_controller_data(nand);
...
static int ma35d0_dev_ready(struct mtd_info *mtd)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ma35d0_nand_info *nand_info = nand_get_controller_data(chip);
	int ready;
...
static int ma35d0_waitfunc(struct mtd_info *mtd, struct nand_chip *chip)
{
...
static void ma35d0_nand_command(struct mtd_info *mtd, unsigned int command,
...
/*
 * ma35d0_nand_read_byte - read a byte from NAND controller into buffer
 * @mtd: MTD device structure
 */
static unsigned char ma35d0_nand_read_byte(struct mtd_info *mtd)
{
	struct nand_chip *nand = mtd_to_nand(mtd);
	struct ma35d0_nand_info *nand_info = nand_get_controller_data(nand);
...
/*
 * ma35d0_nand_write_buf - write data from buffer into NAND controller
 * @mtd: MTD device structure
 * @buf: virtual address in RAM of source
 * @len: number of data bytes to be transferred
 */

static void ma35d0_nand_write_buf(struct mtd_info *mtd, const unsigned char *buf, int len)
{
	struct nand_chip *nand = mtd_to_nand(mtd);
	struct ma35d0_nand_info *nand_info = nand_get_controller_data(nand);
	int i;
...
/*
 * ma35d0_nand_read_buf - read data from NAND controller into buffer
 * @mtd: MTD device structure
 * @buf: virtual address in RAM of source
 * @len: number of data bytes to be transferred
 */
static void ma35d0_nand_read_buf(struct mtd_info *mtd, unsigned char *buf, int len)
{
	struct nand_chip *nand = mtd_to_nand(mtd);
	struct ma35d0_nand_info *nand_info = nand_get_controller_data(nand);
	int i;
 ...
 */
void ma35d0_nand_enable_hwecc(struct mtd_info *mtd, int mode)
{
}
 ...
 */
static int ma35d0_nand_calculate_ecc(struct mtd_info *mtd, const u_char *dat, u_char *ecc_code)
{
	return 0;
}
 ...
 * isnull:     unused
 */
static int ma35d0_nand_correct_data(struct mtd_info *mtd, u_char *dat,
...
static inline int ma35d0_nand_dma_transfer(struct mtd_info *mtd,
 ...
 */
static int ma35d0_nand_write_page_raw(struct mtd_info *mtd,
	struct nand_chip *chip, const uint8_t *buf, int oob_required, int page)
{
	ma35d0_nand_command(mtd, NAND_CMD_SEQIN, 0, page);
	ma35d0_nand_write_buf(mtd, buf, mtd->writesize);
	ma35d0_nand_command(mtd, NAND_CMD_PAGEPROG, -1, -1);
	ma35d0_waitfunc(mtd, chip);
 ...
 * @buf:        data buffer
 */
static int ma35d0_nand_write_page_hwecc(struct mtd_info *mtd,
	struct nand_chip *chip, const uint8_t *buf, int oob_required, int page)
{
	struct ma35d0_nand_info *nand_info = nand_get_controller_data(chip);
	...
	ma35d0_nand_command(mtd, NAND_CMD_SEQIN, 0, page);
	ma35d0_nand_dma_transfer(mtd, buf, mtd->writesize, 0x1);
	ma35d0_nand_command(mtd, NAND_CMD_PAGEPROG, -1, -1);
	ma35d0_waitfunc(mtd, chip);
 ...
 */
static int ma35d0_nand_read_page_raw(struct mtd_info *mtd,
	struct nand_chip *chip, uint8_t *buf, int oob_required, int page)
{
	// read data from nand
	ma35d0_nand_command(mtd, NAND_CMD_READ0, 0, page);
	ma35d0_nand_read_buf(mtd, buf, mtd->writesize);
...
/**
 * ma35d0_nand_read_page_hwecc_oob_first - hardware ecc based page write function
 * @mtd:        mtd info structure
 * @chip:       nand chip info structure
 * @buf:        buffer to store read data
 * @page:       page number to read
 */
static int ma35d0_nand_read_page_hwecc_oob_first(struct mtd_info *mtd,
	struct nand_chip *chip, uint8_t *buf, int oob_required, int page)
{
	struct ma35d0_nand_info *nand_info = nand_get_controller_data(chip);
	uint8_t *p = buf;
	...
	ma35d0_nand_command(mtd, NAND_CMD_READOOB, 0, page);
	ma35d0_nand_read_buf(mtd, chip->oob_poi, mtd->oobsize);
	...
	if ((*(ptr+2) != 0) && (*(ptr+3) != 0))
		...
		ma35d0_nand_command(mtd, NAND_CMD_READ0, 0, page);
		ma35d0_nand_dma_transfer(mtd, p, mtd->writesize, 0x0);
...
/**
 * ma35d0_nand_read_oob_hwecc - the most common OOB data read function
 * @mtd:        mtd info structure
 * @chip:       nand chip info structure
 * @page:       page number to read
 * @sndcmd:     flag whether to issue read command or not
 */
static int ma35d0_nand_read_oob_hwecc(struct mtd_info *mtd, struct nand_chip *chip, int page)
{
	struct nand_chip *nand = mtd_to_nand(mtd);
	struct ma35d0_nand_info *nand_info = nand_get_controller_data(nand);
	char *ptr = (char *)(nand_info->reg+REG_NFI_NANDRA0);
...
int ma35d0_nand_init(struct ma35d0_nand_info *nand_info)
{
	...
	nand->cmd_ctrl = ma35d0_hwcontrol;
	nand->cmdfunc = ma35d0_nand_command;
	nand->waitfunc = ma35d0_waitfunc;
	nand->dev_ready = ma35d0_dev_ready;
	nand->select_chip = ma35d0_nand_select_chip;

	nand->read_byte = ma35d0_nand_read_byte;
	nand->write_buf = ma35d0_nand_write_buf;
	nand->read_buf = ma35d0_nand_read_buf;
	nand->chip_delay = 50;

	nand->ecc.mode       = NAND_ECC_HW_OOB_FIRST;
	nand->ecc.options    = NAND_ECC_CUSTOM_PAGE_ACCESS;
	nand->ecc.hwctl      = ma35d0_nand_enable_hwecc;
	nand->ecc.calculate  = ma35d0_nand_calculate_ecc;
	nand->ecc.correct    = ma35d0_nand_correct_data;
	nand->ecc.write_page = ma35d0_nand_write_page_hwecc;
	nand->ecc.read_page  = ma35d0_nand_read_page_hwecc_oob_first;
	nand->ecc.read_oob   = ma35d0_nand_read_oob_hwecc;
	nand->ecc.layout     = &ma35d0_nand_oob;
	nand->ecc.write_page_raw = ma35d0_nand_write_page_raw;
	nand->ecc.read_page_raw  = ma35d0_nand_read_page_raw;
	...
	if (nand->ecc.strength == 0) {
		...
		ma35d0_layout_oob_table(&ma35d0_nand_oob, mtd->oobsize,
	...
	nand->ecc.bytes = ma35d0_nand_oob.eccbytes / nand->ecc.steps;
	nand->ecc.total = ma35d0_nand_oob.eccbytes;
...
static int ma35d0_nand_probe(struct udevice *dev)
{
	struct ma35d0_nand_info *info = dev_get_priv(dev);
	struct resource res;
	int ret;
	...
	return ma35d0_nand_init(info);
}

static const struct udevice_id ma35d0_nand_ids[] = {
	{ .compatible = "nuvoton,ma35d0-nand" },
	{ }
};

U_BOOT_DRIVER(ma35d0_nand) = {
	.name = "ma35d0-nand",
	.id = UCLASS_MTD,
	.of_match = ma35d0_nand_ids,
	.probe = ma35d0_nand_probe,
	.priv_auto_alloc_size = sizeof(struct ma35d0_nand_info),
};
...
void board_nand_init(void)
{
	...
	/* enable nand clock */
	ret = uclass_get_device_by_driver(UCLASS_CLK, DM_GET_DRIVER(ma35d0_clk), &dev);
	if (ret)
	...
	ret = uclass_get_device_by_driver(UCLASS_MTD,
					  DM_GET_DRIVER(ma35d0_nand),
					  &dev);
	if (ret && ret != -ENODEV)
		pr_err("Failed to initialize ma35d0 NAND controller. (error %d)\n", ret);
}
