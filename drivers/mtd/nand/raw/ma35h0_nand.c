...
static struct nand_ecclayout ma35h0_nand_oob;
...
static void ma35h0_layout_oob_table(struct nand_ecclayout *pNandOOBTbl,
...
static void ma35h0_hwcontrol(struct mtd_info *mtd, int cmd, unsigned int ctrl)
{
	struct nand_chip *nand = mtd_to_nand(mtd);
	struct ma35h0_nand_info *nand_info = nand_get_controller_data(nand);
...
/* select chip */
static void ma35h0_nand_select_chip(struct mtd_info *mtd, int chip)
{
	struct nand_chip *nand = mtd_to_nand(mtd);
	struct ma35h0_nand_info *nand_info = nand_get_controller_data(nand);
...
static int ma35h0_dev_ready(struct mtd_info *mtd)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct ma35h0_nand_info *nand_info = nand_get_controller_data(chip);
	int ready;
...
static int ma35h0_waitfunc(struct mtd_info *mtd, struct nand_chip *chip)
{
	struct ma35h0_nand_info *nand = nand_get_controller_data(chip);
	unsigned long time;
	int status = -1;
...
static void ma35h0_nand_command(struct mtd_info *mtd, unsigned int command,
...
/*
 * ma35h0_nand_read_byte - read a byte from NAND controller into buffer
 * @mtd: MTD device structure
 */
static unsigned char ma35h0_nand_read_byte(struct mtd_info *mtd)
{
	struct nand_chip *nand = mtd_to_nand(mtd);
	struct ma35h0_nand_info *nand_info = nand_get_controller_data(nand);
...
/*
 * ma35h0_nand_write_buf - write data from buffer into NAND controller
 * @mtd: MTD device structure
 * @buf: virtual address in RAM of source
 * @len: number of data bytes to be transferred
 */

static void ma35h0_nand_write_buf(struct mtd_info *mtd, const unsigned char *buf, int len)
{
	struct nand_chip *nand = mtd_to_nand(mtd);
	struct ma35h0_nand_info *nand_info = nand_get_controller_data(nand);
	int i;
...
/*
 * ma35h0_nand_read_buf - read data from NAND controller into buffer
 * @mtd: MTD device structure
 * @buf: virtual address in RAM of source
 * @len: number of data bytes to be transferred
 */
static void ma35h0_nand_read_buf(struct mtd_info *mtd, unsigned char *buf, int len)
{
	struct nand_chip *nand = mtd_to_nand(mtd);
	struct ma35h0_nand_info *nand_info = nand_get_controller_data(nand);
	int i;
 ...
 */
void ma35h0_nand_enable_hwecc(struct mtd_info *mtd, int mode)
{
}
 ...
 */
static int ma35h0_nand_calculate_ecc(struct mtd_info *mtd, const u_char *dat, u_char *ecc_code)
{
	return 0;
}
 ...
 * isnull:     unused
 */
static int ma35h0_nand_correct_data(struct mtd_info *mtd, u_char *dat,
...
void fmiSM_CorrectData_BCH(struct mtd_info *mtd, u8 ucFieidIndex, u8 ucErrorCnt, u8 *pDAddr)
{
	...
	for (ii = 0; ii < jj; ii++) {
		...
		uaAddr[ii*2+0] = readl(nand_info->reg+REG_NFI_NANDECCEA0 + ii*4) & 0x07ff;
	...
	for (ii = 0; ii < ucErrorCnt; ii++) {
		...
		// for wrong parity code in redundancy area
			...
			smra_index = (u8 *)(nand_info->reg+REG_NFI_NANDRA0 +
					(readl(nand_info->reg+REG_NFI_NANDRACTL) & 0x1ff) -
...
static inline int ma35h0_nand_dma_transfer(struct mtd_info *mtd,
	...
	if (is_write) {
		...
		if (readl(nand_info->reg+REG_NFI_NANDCTL) & 0x80) {
			do {
				...
				stat = fmiSMCorrectData(mtd, (unsigned long)addr);
				if (stat < 0) {
				...
				(readl(nand_info->reg+REG_NFI_NANDINTSTS) & 0x4));
		} else
			while (!(readl(nand_info->reg+REG_NFI_NANDINTSTS) & 0x1))
				;
	}
 ...
 */
static int ma35h0_nand_write_page_raw(struct mtd_info *mtd,
	struct nand_chip *chip, const uint8_t *buf, int oob_required, int page)
{
	ma35h0_nand_command(mtd, NAND_CMD_SEQIN, 0, page);
	ma35h0_nand_write_buf(mtd, buf, mtd->writesize);
	ma35h0_nand_command(mtd, NAND_CMD_PAGEPROG, -1, -1);
	ma35h0_waitfunc(mtd, chip);
 ...
 * @buf:        data buffer
 */
static int ma35h0_nand_write_page_hwecc(struct mtd_info *mtd,
	struct nand_chip *chip, const uint8_t *buf, int oob_required, int page)
{
	struct ma35h0_nand_info *nand_info = nand_get_controller_data(chip);
	...
	ma35h0_nand_command(mtd, NAND_CMD_SEQIN, 0, page);
	ma35h0_nand_dma_transfer(mtd, buf, mtd->writesize, 0x1);
	ma35h0_nand_command(mtd, NAND_CMD_PAGEPROG, -1, -1);
	ma35h0_waitfunc(mtd, chip);
 ...
 */
static int ma35h0_nand_read_page_raw(struct mtd_info *mtd,
	struct nand_chip *chip, uint8_t *buf, int oob_required, int page)
{
	// read data from nand
	ma35h0_nand_command(mtd, NAND_CMD_READ0, 0, page);
	ma35h0_nand_read_buf(mtd, buf, mtd->writesize);
...
/**
 * ma35h0_nand_read_page_hwecc_oob_first - hardware ecc based page write function
 * @mtd:        mtd info structure
 * @chip:       nand chip info structure
 * @buf:        buffer to store read data
 * @page:       page number to read
 */
static int ma35h0_nand_read_page_hwecc_oob_first(struct mtd_info *mtd,
	struct nand_chip *chip, uint8_t *buf, int oob_required, int page)
{
	struct ma35h0_nand_info *nand_info = nand_get_controller_data(chip);
	uint8_t *p = buf;
	...
	ma35h0_nand_command(mtd, NAND_CMD_READOOB, 0, page);
	ma35h0_nand_read_buf(mtd, chip->oob_poi, mtd->oobsize);
	...
	if ((*(ptr+2) != 0) && (*(ptr+3) != 0))
		...
		ma35h0_nand_command(mtd, NAND_CMD_READ0, 0, page);
		ma35h0_nand_dma_transfer(mtd, p, mtd->writesize, 0x0);
...
/**
 * ma35h0_nand_read_oob_hwecc - the most common OOB data read function
 * @mtd:        mtd info structure
 * @chip:       nand chip info structure
 * @page:       page number to read
 * @sndcmd:     flag whether to issue read command or not
 */
static int ma35h0_nand_read_oob_hwecc(struct mtd_info *mtd, struct nand_chip *chip, int page)
{
	struct nand_chip *nand = mtd_to_nand(mtd);
	struct ma35h0_nand_info *nand_info = nand_get_controller_data(nand);
	char *ptr = (char *)(nand_info->reg+REG_NFI_NANDRA0);
...
int ma35h0_nand_init(struct ma35h0_nand_info *nand_info)
{
	...
	nand->cmd_ctrl = ma35h0_hwcontrol;
	nand->cmdfunc = ma35h0_nand_command;
	nand->waitfunc = ma35h0_waitfunc;
	nand->dev_ready = ma35h0_dev_ready;
	nand->select_chip = ma35h0_nand_select_chip;

	nand->read_byte = ma35h0_nand_read_byte;
	nand->write_buf = ma35h0_nand_write_buf;
	nand->read_buf = ma35h0_nand_read_buf;
	nand->chip_delay = 50;

	nand->ecc.mode       = NAND_ECC_HW_OOB_FIRST;
	nand->ecc.options    = NAND_ECC_CUSTOM_PAGE_ACCESS;
	nand->ecc.hwctl      = ma35h0_nand_enable_hwecc;
	nand->ecc.calculate  = ma35h0_nand_calculate_ecc;
	nand->ecc.correct    = ma35h0_nand_correct_data;
	nand->ecc.write_page = ma35h0_nand_write_page_hwecc;
	nand->ecc.read_page  = ma35h0_nand_read_page_hwecc_oob_first;
	nand->ecc.read_oob   = ma35h0_nand_read_oob_hwecc;
	nand->ecc.layout     = &ma35h0_nand_oob;
	nand->ecc.write_page_raw = ma35h0_nand_write_page_raw;
	nand->ecc.read_page_raw  = ma35h0_nand_read_page_raw;
	...
	// Enable SM_CS0
	writel((readl(nand_info->reg+REG_NFI_NANDCTL) & (~0x02000000))|0x4000000,
		nand_info->reg+REG_NFI_NANDCTL);
	writel(0x1, nand_info->reg+REG_NFI_NANDECTL); /* un-lock write protect */

	// NAND Reset
	writel(readl(nand_info->reg+REG_NFI_NANDCTL) | 0x1,
		nand_info->reg+REG_NFI_NANDCTL);    // software reset
	...
	if (nand->ecc.strength == 0) {
		...
		ma35h0_layout_oob_table(&ma35h0_nand_oob, mtd->oobsize,
			g_i32ParityNum[mtd->writesize>>12][nand_info->eBCHAlgo]);
	...
	nand->ecc.bytes = ma35h0_nand_oob.eccbytes / nand->ecc.steps;
	nand->ecc.total = ma35h0_nand_oob.eccbytes;
	...
	// Set BCH algorithm
	writel((readl(nand_info->reg+REG_NFI_NANDCTL) & (~0x007C0000)) |
		g_i32BCHAlgoIdx[nand_info->eBCHAlgo], nand_info->reg+REG_NFI_NANDCTL);
...
static int ma35h0_nand_probe(struct udevice *dev)
{
	struct ma35h0_nand_info *info = dev_get_priv(dev);
	struct resource res;
	int ret;
	...
	return ma35h0_nand_init(info);
}

static const struct udevice_id ma35h0_nand_ids[] = {
	{ .compatible = "nuvoton,ma35h0-nand" },
	{ }
};

U_BOOT_DRIVER(ma35h0_nand) = {
	.name = "ma35h0-nand",
	.id = UCLASS_MTD,
	.of_match = ma35h0_nand_ids,
	.probe = ma35h0_nand_probe,
	.priv_auto_alloc_size = sizeof(struct ma35h0_nand_info),
};
...
void board_nand_init(void)
{
	...
	/* enable nand clock */
	ret = uclass_get_device_by_driver(UCLASS_CLK, DM_GET_DRIVER(ma35h0_clk), &dev);
	if (ret)
	...
	ret = uclass_get_device_by_driver(UCLASS_MTD,
					  DM_GET_DRIVER(ma35h0_nand),
					  &dev);
	if (ret && ret != -ENODEV)
		pr_err("Failed to initialize ma35h0 NAND controller. (error %d)\n", ret);
}
